...
if(imageInput != null) { // 프로필 변경 화면인 경우
  ...
  const updatePreview = (file) => {
    ...
    // 파일을 읽어 오는데
    // DataURL 형식으로 읽어옴
    // DataURL : 파일 전체 데이터가 브라우저가 해석할수있는 
    //           긴 주소형태 문자열로 변환
  ...
  if(file === undefined){
    ...
    // 이전에 선택한 파일이 없는 경우
    // == 현재 페이지 들어와서 프로필 이미지를 바꾼 적이 없는 경우
    ...
    // DataTransfer 객체가 가지고 있는 files 필드에
    // lastValidFile 추가
    dataTransfer.items.add(lastValidFile);

    // input의 files 필드에 lastValidFile이 추가된 files 대입
