...
emailMessageObj.invalid     = "알맞은 이메일 형식으로 작성해주세요";
...
memberEmail.addEventListener("input" , e => {
  ...
  if( regEx.test(inputEmail) === false ){
    ...
    emailMessage.classList.remove("confirm"); // 녹색 글씨 제거
  ...
  .then(count => {
    ...
    // 중복이 아닌 경우
    emailMessage.innerText = emailMessageObj.check; // 중복 메시지
    emailMessage.classList.add("confirm"); // 빨간 글씨 추가
    emailMessage.classList.remove("error"); // 녹색 글씨 제거
...
memberNickname.addEventListener("input", () => {
  ...
  if( regEx.test(inputNickname) === false ){
    ...
    nickMessage.classList.remove("confirm"); // 녹색 글씨 제거
  ...
  .then(count => {
    ...
    nickMessage.innerText = nickMessageObj.check; // 중복 메시지
    nickMessage.classList.remove("error"); // 녹색 글씨 제거
    nickMessage.classList.add("confirm"); // 빨간 글씨 추가
    checkObj.memberNickname = true; //  유효한 닉네임임을 기록
...
memberTel.addEventListener("input", () => {
  ...
  if(inputTel.length === 0){
    ...
    telMessage.classList.remove("comfirm", "error");
...
// 인증 관련 메시지 출력 span 태그(인증 번호 입력칸 아래에 남은 시간 등)
...
let authTimer; // 타이머 역할의 setInterval를 저장할 변수
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  //    Redis에 저장된 이메일, 인증 번호와 일치하는지 확인

  /* AJAX로 여러 데이터를 서버로 전달하고 싶을 때
     JSON 형태로 값을 전달해야 한다! */
