...
public class MemberServiceImpl implements MemberService{
	...
	public int signUp(Member inputMember) {
		...
		// 1) 비밀번호 암호화(BCrypt)
		String encPw = encoder.encode(inputMember.getMemberPw()); // 입력된 암호를 get한 다음 암호화. common.config폴더에 SecurityConfig
		inputMember.setMemberPw(encPw);
		
		// 2) 주소 미입력 시 null로 변경
		if(inputMember.getMemberAddress().equals(",,")) { // 주소가 입력 되지 않으면 값이 ",," 으로 나옴
