...
public class EditBoardServiceImpl implements EditBoardService{
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) {
		...
		for(int i = 0 ; i < images.size(); i++) {
			...
			BoardImg img 
				= BoardImg.builder()
					...
					.uploadFile(images.get(i))		 // 실제 업로드 된 이미지 데이터
					.build();							
		 ...
		 * 1. 1행을 삽입하는 mapper 메서드를 여러 번 호출하는 방법 
		...
		// INSERT 된 행의 개수와  uploadList의 개수가 같지 않은 경우
		if(insertRows != uploadList.size()) {
			throw new RuntimeException("이미지 INSERT 실패!");
			// 앞서 삽입한 게시글 부분도 ROLLBACK되도록 예외 강제 발생
