...
//	 자동으로 같은 자료형의 Bean을 의존성 주입(DI)함
...
public class EmailServiceImpl implements EmailService{
	...
	// 자바에서 타임리프를 사용할 수 있게 하는 객체(Bean)
	...
	public int sendEmail(String htmlName, String email) {
		...
		try {
			...
			String emailTitle = null; // emailTitle : 발송되는 이메일의 제목 역할을 하는 변수
			String authKey    = createAuthKey(); // authKey    : 생성된 인증 번호가 저장되는 변수 
			
			// 이메일 발송 시 사용할 html 파일의 이름에 따라
			// 이메일 제목, 내용을 다르게 설정
			switch(htmlName) {
			case "signUp" : 
				emailTitle = "[boardProject] 회원 가입 인증 번호 입니다";
				break;
			...
			// HTML 코드 전체가 하나의 String으로 변환돼서 반환 받은 후
...
// 의존성 주입 방법 2 : setter 이용 (좀 더 안전한 방법, 같은 자료형의 필드가 여러 개 일때 주로 사용)
//	private 자료형 필드명;
//	@Autowired
...
/* 제일 추천하는 방법 */
// 의존성 주입 방법 3 : 생성자를 이용하는 방법 (필드가 많을 때)
